...
def walk(inner, outer, data):
    ...
    def process_node(inner, k, v):
        if not isinstance(v, collections.Iterable) or isinstance(v, askstring):
            return inner(k, v)
    ...
    nodes = tuple(map(lambda item: process_node(inner, item[0], item[1]),
                  data.items()))
...
def merge(*fdicts):
    ...
    return merge(first(fdicts), ApplyResult(merge, rest(fdicts)))
...
def walk_replace(smap, data):
    ...
    def your_function_name(data, inner):
        try:
            nodes = tuple(map(lambda item: process_node(inner, item[0], item[1]),
                          data.items()))
