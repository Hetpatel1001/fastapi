...
def walk_pair(inner, outer, data, schema, handler):
    ...
    def process_node(inner, k, v1, v2):
        if (not isinstance(v1, Iterable)
            or isinstance(v1, str)
            or ((isinstance(v1, Sequence)
...
def sanitize(data, schema):
    def sanitize_node(k, v, validator):
        if validator(v) or validator == any:
            ...
            print(f"Schema violation for key '{k}' and value '{v}'")
            return None

    def handler(k):
        print(f"Cannot validate '{k}', key not in schema")
        return None
...
def validate(data, schema):
    ...
    def handler(k):
        ...
        if type_ == required_key:
            raise MarshallingError(f"Field missing: {key}")
        return None
...
def validate_with(schema):
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not kwargs.get('form_params'):
                form_params = {}
            else:
                form_params = validate(kwargs['form_params'], schema)
            kwargs.update({'query_params': query_params, 'form_params': form_params})
...
def marshal(data, schema, before=False):
    def marshal_node(k, v1, v2, before=False):
        ...
        try:
            ...
            raise MarshallingError(f"Cannot process node for key '{k}' and value '{v1}'")

    def handler(k):
        # print(f"Cannot process '{k}', key not in schema")
        return None
    return walk_pair(partial(marshal_node, before=before), identity, data, schema, handler)
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            if not kwargs.get('query_params'):
                ...
                query_params = marshal(kwargs['query_params'], schema, before=True)
            if not kwargs.get('form_params'):
                form_params = {}
            else:
                form_params = marshal(kwargs['form_params'], schema, before=True)
            kwargs.update({'query_params': query_params, 'form_params': form_params})
            response_map = f(*args, **kwargs)
